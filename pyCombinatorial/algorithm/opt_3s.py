...
# Lesson: Local Search-3-opt Stochastic
...
def segments_3_opt_stochastic(n, search):
    x       = []
    a, b, c = 0, 0, 0
    for i in range(0, n):
        a = i
...
# Function: 3_opt
def local_search_3_opt_stochastic(distance_matrix, city_tour, recursive_seeding = -1, search = 1000, verbose = True):
    ...
    while (count < recursive_seeding):
        ...
        x            = segments_3_opt_stochastic(len(city_list[0]), search)
        for item in x:
            i, j, k = item   
            ...
            trial   = [ 
                        ...
                        # 3-opt: Two Subsequent 2-opt
                        [A + b + c], [a + b + C], [a + B + c], 
                        
                        # 3-opt: Three Subsequent 2-opt
                        [a + b + c]  
                      ] 
