...
# Lesson: Local Search-2.5-opt Stochastic
...
def segments_2_opt_stochastic(n, search):
    x    = []
    a, b = 0, 0
    for i in range(0, n):
        a = i
    ...
    if (search > len(x)):
        search = len(x)
    x = random.sample(x, search)
    return x

############################################################################

# Function: 2_5_opt Stochastic
def local_search_2h_opt_stochastic(distance_matrix, city_tour, recursive_seeding = -1, search = 1000, verbose = True):
    ...
    while (count < recursive_seeding):
        ...
        x            = segments_2_opt_stochastic(len(city_list[0]), search)
        for item in x:
            trial                = []
            i, j                 = item   
