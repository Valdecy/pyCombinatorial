...
# Lesson: pyCombinatorial - Cheapest Insertion
...
def cheapest_insertion(distance_matrix, local_search = True, verbose = True):
    n      = distance_matrix.shape[0]
    dm     = distance_matrix.astype(float).copy()
    np.fill_diagonal(dm, -np.inf)
    i, idx = np.unravel_index(np.argmax(dm, axis = None), dm.shape)
    temp   = [i, idx]
    count  = 0
    while (len(temp) < n):            
        remaining  = [k for k in range(n) if k not in temp]
        ...
        for k in remaining:
            for p in range(0, len(temp)):
                ...
                if delta < best_delta:
                    ...
                    best_city  = k
                    best_pos   = p + 1
        temp.insert(best_pos, best_city)
        if (verbose == True):
