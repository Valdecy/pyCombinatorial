...
# Function: SARSA
def sarsa(distance_matrix, learning_rate = 0.1, discount_factor = 0.95, epsilon = 0.15, episodes = 5000, q_init = None, local_search = True, verbose = True):
    ...
    for episode in range(0, episodes):
        ...
        while (len(visited) < num_cities):
            ...
            if (len(unvisited_cities) == 0):
                ...
                next_next_city = random.choice(unvisited_cities)
            else:
                next_next_city = unvisited_cities[np.argmax(q_table[next_city, unvisited_cities])]
            if (next_next_city is not None):
                q_table[current_city, next_city] = q_table[current_city, next_city] + learning_rate * (reward + discount_factor * q_table[next_city, next_next_city] - q_table[current_city, next_city])
            else:
                q_table[current_city, next_city] = q_table[current_city, next_city] + learning_rate * (reward - q_table[current_city, next_city])
            current_city = next_city
            next_city    = next_next_city
