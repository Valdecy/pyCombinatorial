...
# Lesson: Local Search-5-opt Stochastic
...
def segments_5_opt_stochastic(n, search):
    x             = []
    a, b, c, d, e = 0, 0, 0, 0, 0
    for i in range(0, n):
        a = i
...
def local_search_5_opt_stochastic(distance_matrix, city_tour, recursive_seeding = -1, search = 1000, verbose = True):
    ...
    while (count < recursive_seeding):
        ...
        x            = segments_5_opt_stochastic(len(city_list[0]), search)
        for item in x:
            i, j, k, L, m = item   
