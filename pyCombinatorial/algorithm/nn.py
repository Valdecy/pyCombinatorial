...
def nearest_neighbour(distance_matrix, initial_location = -1, local_search = True, verbose = True):
    minimum  = float('+inf')
    distance = float('+inf')
    route    = []
    for i1 in range(0, distance_matrix.shape[0]):
        ...
        temp.append(i1)
        temp.append(idx)
        for _ in range(0, distance_matrix.shape[0]-2):
            i2         = idx
            idx        = dist[i2,:].argmin()  
            dist[i2,:] = float('+inf')
            dist[:,i2] = float('+inf')
        ...
        if (local_search == True):
            temp, val = local_search_2_opt(distance_matrix, [temp, val], recursive_seeding = -1, verbose = False)
        if (val < minimum):
            minimum  = val
            distance = val
