...
# Lesson: pyCombinatorial - Stochastic Hill Clibing
...
# Function: Mutation
def mutate_candidate(Xdata, city_tour):
    city_list = copy.deepcopy(city_tour)
    best_route = copy.deepcopy(city_list)  
    rand = int.from_bytes(os.urandom(8), byteorder = "big") / ((1 << 64) - 1) 
    if (rand < 0.5):    
        m, n = random.sample(range(0, len(city_tour[0])-2), 2)
        if (m > n):
            m, n = n, m
...
def stochastic_hill_climbing(Xdata, city_tour, iterations = 50):
    count = 0
    best_solution = copy.deepcopy(city_tour)  
    candidate = copy.deepcopy(city_tour) 
    while (count < iterations):
        candidate = mutate_candidate(Xdata, city_tour = candidate)              
